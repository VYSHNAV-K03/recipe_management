...
function ShowRecipe() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios(`http://localhost:8000/api/recipes/${id}`);
    ...
    <Layout>
      <Box className="md:grid md:grid-cols-3 md:gap-y-10 mt-20">
        <div className="flex flex-col gap-2 md:gap-0 md:flex-none md:grid md:grid-cols-2 px-3 col-span-3 mb-5 md:mb-0">
          ...
          <Button
            onClick={() => destroy()}
            className="px-5  col-start-3  md:w-80 md:h-16 bg-red-400/70 text-white hover:bg-red-700 hover:text-white md:transform md:transition-all md:hover:scale-105"
          >
        ...
        <div className="flex justify-center items-center md:col-start-3">
          <Typography alignItems="center" className="flex" component="legend">
            {" "}
        ...
        <div className="col-start-2 col-span-2 lg:mr-4 p-10 backdrop-blur-sm bg-zinc-900/30">
          ...
          <ul className="list-decimal gap-5">{direction}</ul>
        </div>

        <div className="mt-10 mb-20 col-start-2 flex justify-center w-full">
          <NavLink to={"/recipes"}>
            <Button className="text-white w-56 h-16 bg-violet-400/70  hover:bg-violet-700 md:transform md:transition-all md:hover:scale-105">
              Back to all recipes
            </Button>
          </NavLink>
        </div>
